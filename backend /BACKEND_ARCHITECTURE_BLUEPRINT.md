...
- future rate limiting, sanitization, caching, and error middleware
...
Middleware handles cross-cutting concerns before controller execution: JWT verification, upload parsing, future validation middleware, future rate limiting, and future security middleware.
...
Not currently implemented. Future projects should add rate limiting for auth, OTP, public, and upload endpoints.
...
Current implemented controls include CORS, JWT, file type filters, file size limits, password hashing dependency availability, environment variables, and schema validation. Future projects should add Helmet, input sanitization, and rate limiting.
...
- Add rate limiting for OTP, login, public APIs, and uploads.
